...
const EmailProviderService = {
  emailProviderList: async (data) => {
    try {
      ...
      const endPoint = apiConfig.endPoints.EMAIL_PROVIDER.VIEW;
  ...
  deleteProvider: async (providerUID) => {
    try {
      const endPoint = replaceText(
        apiConfig.endPoints.EMAIL_PROVIDER.DELETE,
        ':providerUID',
        providerUID
      );
      const apiURL = apiConfig.baseURL.API_BASE_URL + endPoint;
  ...
  changeProviderStatus: async (providerUID) => {
    try {
      const endPoint = replaceText(
        apiConfig.endPoints.EMAIL_PROVIDER.STATUS,
        ':providerUID',
        providerUID
      );
      const apiURL = apiConfig.baseURL.API_BASE_URL + endPoint;
  ...
  getProviderDetails: async (providerUID) => {
    try {
      const endPoint = replaceText(
        apiConfig.endPoints.EMAIL_PROVIDER.DETAIL,
        ':providerUID',
        providerUID
      );
      const apiURL = apiConfig.baseURL.API_BASE_URL + endPoint;
  ...
  updateProvider: async (providerUID, data) => {
    try {
      const endPoint = replaceText(
        apiConfig.endPoints.EMAIL_PROVIDER.EDIT,
        ':providerUID',
        providerUID
      );
      const apiURL = apiConfig.baseURL.API_BASE_URL + endPoint;
