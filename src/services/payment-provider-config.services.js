...
const PaymentProviderService = {
  paymentProviderList: async (data) => {
    try {
      ...
      const endPoint = apiConfig.endPoints.PAYMENT_PROVIDER_CONFIG.VIEW;
  ...
  deleteProvider: async (providerUID) => {
    try {
      const endPoint = replaceText(
        apiConfig.endPoints.PAYMENT_PROVIDER_CONFIG.DELETE,
        ':providerUID',
        providerUID
      );
      const apiURL = apiConfig.baseURL.API_BASE_URL + endPoint;
  ...
  changeProviderStatus: async (providerUID) => {
    try {
      const endPoint = replaceText(
        apiConfig.endPoints.PAYMENT_PROVIDER_CONFIG.STATUS,
        ':providerUID',
        providerUID
      );
      const apiURL = apiConfig.baseURL.API_BASE_URL + endPoint;
  ...
  getProviderDetails: async (providerUID) => {
    try {
      const endPoint = replaceText(
        apiConfig.endPoints.PAYMENT_PROVIDER_CONFIG.DETAIL,
        ':providerUID',
        providerUID
      );
      const apiURL = apiConfig.baseURL.API_BASE_URL + endPoint;
