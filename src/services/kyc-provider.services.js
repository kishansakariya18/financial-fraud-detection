import { statusToAPI } from 'app/pages/kyc-provider/helper';
import apiConfig from '../configs/api.config';
import { sendRequest } from '../utils/axios';
import { DEFAULT_PER_PAGE_RECORD } from 'constants/app.constant';
import { replaceText } from 'utils/custom.utilities';

const KYCProviderService = {
  kycProviderList: async (data) => {
    try {
      ...
      const endPoint = apiConfig.endPoints.KYC_PROVIDER.VIEW;
  ...
  deleteProvider: async (providerUID) => {
    try {
      const endPoint = replaceText(
        apiConfig.endPoints.KYC_PROVIDER.DELETE,
        ':providerUID',
        providerUID
      );
      const apiURL = apiConfig.baseURL.API_BASE_URL + endPoint;
  ...
  changeProviderStatus: async (providerUID) => {
    try {
      const endPoint = replaceText(
        apiConfig.endPoints.KYC_PROVIDER.STATUS,
        ':providerUID',
        providerUID
      );
      const apiURL = apiConfig.baseURL.API_BASE_URL + endPoint;
  ...
  getProviderDetails: async (providerUID) => {
    try {
      const endPoint = replaceText(
        apiConfig.endPoints.KYC_PROVIDER.DETAIL,
        ':providerUID',
        providerUID
      );
      const apiURL = apiConfig.baseURL.API_BASE_URL + endPoint;
  ...
  updateProvider: async (providerUID, data) => {
    try {
      const endPoint = replaceText(
        apiConfig.endPoints.KYC_PROVIDER.EDIT,
        ':providerUID',
        providerUID
      );
      const apiURL = apiConfig.baseURL.API_BASE_URL + endPoint;
...
export default KYCProviderService;
