...
export function ViewDetails({ transactionData, onClose }) {
  ...
  // Use the passed transaction data
        ...
        <div className="col-span-12 sm:col-span-8 lg:col-span-9">
          ...
          <div className="mt-4 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            <div>
              ...
              <p className="font-medium">{response?.TransactionUID}</p>
            </div>
            <div>
              <p className="text-sm font-medium text-gray-800 dark:text-dark-100">
                {t('user') + ' ID'}
              </p>
              <p className="font-medium">{response?.UserID || 'Not provided'}</p>
            </div>
            <div>
              <p className="text-sm font-medium text-gray-800 dark:text-dark-100">
                {t('transaction') + ' ' + t('type')}
              </p>
              <p className="font-medium">{transactionData?.transactionType || 'N/A'}</p>
            </div>
            <div>
              <p className="text-sm font-medium text-gray-800 dark:text-dark-100">
            ...
            <div>
              ...
              <p className="font-medium">{response?.ReferenceID || 'N/A'}</p>
            </div>
            <div>
            ...
            <div>
              ...
              <p className="font-medium">{response?.CreditDebitType === 0 ? 'Credit' : 'Debit'}</p>
            </div>
            <div>
              <p className="text-sm font-medium text-gray-800 dark:text-dark-100">
                {t('createdAt')}
              </p>
              <p className="font-medium">{getDateInUTCToTimeZone(response?.DateCreated)}</p>
            </div>
          </div>
          <div className="mt-4">
