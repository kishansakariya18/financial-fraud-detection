...
const TimelineItem = ({
  ...
  return (
    <div className={clsx('timeline-item', className, classNames?.root)} {...rest}>
      ...
      <div className={`timeline-item-content-wrappper ${classNames?.contentWrapper}`}>
        <div className="flex flex-col pb-1.5">
          ...
          <span className={clsx('text-xs text-gray-400 dark:text-dark-300', classNames?.time)}>
            {formattedTime}
          </span>
        </div>
        <div className={clsx('timeline-item-content py-1', classNames?.content)}>{children}</div>
      </div>
    </div>
  );
};
