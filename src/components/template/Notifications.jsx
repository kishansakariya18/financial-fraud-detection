...
export function Notifications() {
  ...
  const clearNotifications = () => {
    if (activeTab === 0) {
      ...
      setNotifications((n) => n.filter((n) => n.type !== typesKey[activeTab - 1]));
    }
  };

  return (
    <Popover className="relative flex">
      <PopoverButton as={Button} variant="flat" isIcon className="relative size-9 rounded-full">
        <AlarmIcon className="size-6 text-gray-900 dark:text-dark-100" />
        {notifications.length > 0 && (
          <AvatarDot color="error" isPing className="top-0 ltr:right-0 rtl:left-0" />
        )}
      </PopoverButton>
      <Transition
              ...
              <div className="rounded-t-lg bg-gray-100 dark:bg-dark-800">
                <div className="flex items-center justify-between px-4 pt-2">
                  <div className="flex items-center gap-2">
                    <h3 className="font-medium text-gray-800 dark:text-dark-100">Notifications</h3>
                    {notifications.length > 0 && (
                  ...
                  <TabPanels as={Fragment}>
                    <TabPanel className="custom-scrollbar grow space-y-4 overflow-y-auto overflow-x-hidden p-4 outline-none">
                      {notifications.map((item) => (
                        <NotificationItem key={item.id} remove={removeNotification} data={item} />
                      ))}
                    </TabPanel>
                    {typesKey.map((key) => (
                      <TabPanel
    ...
    <div className="group flex items-center justify-between gap-3">
      <div className="flex min-w-0 gap-3">
        ...
        <div className="min-w-0">
          ...
          <div className="mt-1 truncate text-xs text-gray-400 dark:text-dark-300">{data.time}</div>
        </div>
      </div>
      <Button
        variant="flat"
