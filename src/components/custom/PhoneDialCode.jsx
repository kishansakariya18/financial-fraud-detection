...
export function PhoneDialCode({ onChange, value, name, error, countries }) {
  const {
  ...
  return (
    <Combobox
      as="div"
      value={countries.find((country) => country.phoneCode === value) || null}
      onChange={(val) => onChange(val?.phoneCode)}
      name={name}>
