...
export function FileItemSquare({ file, handleRemove, imageLink, ...rest }) {
  console.log('file::', imageLink);
  let isImage;
  let src;
  if (file) {
  ...
  return (
    <div
      ...
      {...rest}>
      {isImage ? (
        <img className="h-full w-full object-contain" src={src} alt={file?.name} />
      ) : (
        <div className="h-full w-full select-none rounded-lg bg-gray-150 px-1 py-2 text-center dark:bg-dark-900">
          ...
          <span className="mt-1.5 line-clamp-2 text-tiny">{file?.name}</span>
        </div>
      )}
