...
export function Profile() {
  ...
  const [confirmState, setConfirmState] = useState('pending'); // 'pending' | 'success' | 'error'
  ...
  const handleConfirmClose = () => {
    if (confirmLoading) return; // prevent closing while loading
  ...
  const handleConfirmOk = () => {
    ...
    req
      .then(() => {
        ...
        // Hard refresh to login route; keep modal visible until navigation
        const loginPath = isAgentuser ? '/agent-auth/login' : '/login';
    ...
    <>
      ...
      <ConfirmModal
        ...
        messages={{
          pending: {
            title: t('logout'),
            description: t('areYouSure') || 'Are you sure you want to logout?',
            actionText: t('logout')
          },
          error: {
            title: t('error') || 'Opps... Something failed.',
            description: (
              <>
                {t('logout_failed_try_again') ||
