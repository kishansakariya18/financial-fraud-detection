...
const CloneCampaign = () => {
  ...
  const breadcrumbItem = [{ title: t('campaign'), path: '/campaign' }, { title: t('clone') }];
  ...
  useEffect(() => {
    const fetchCampaignDetail = async () => {
      ...
      if (result?.status === 200) {
        ...
        if (data) {
          reset({
            campaignName: '', // Keep campaign name empty for clone
  ...
  useEffect(() => {
    if (!loading && !error && response) {
      toast.success(response.message || 'Campaign cloned successfully');
      setResponse(null);
      navigate('/campaign');
    }
    // eslint-disable-next-line react-hooks/exhaustive-deps
            ...
            <div className="rounded-lg border border-gray-200 bg-white p-6 dark:border-dark-600 dark:bg-dark-800">
              ...
              <div className="space-y-4">
                <Input
                  ...
                  placeholder="e.g. August Kickoff Reloads (Clone)"
                />
