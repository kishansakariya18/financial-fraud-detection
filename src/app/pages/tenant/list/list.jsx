...
import { tenantListResponseMapper } from '../helper';
import TenantService from 'services/tenant.services';

export default function TenantList() {
  const { t } = useTranslation();
  const [searchParams, setSearchParams] = useSearchParams();
  const pageTitle = t('tenant') + ' ' + t('list');
  ...
  const fetchTenantList = async () => {
    ...
    if (result.status === 200) {
      return {
        status: 200,
        data: tenantListResponseMapper(result.response.data),
  ...
  const { table, isLoading, error, setError, tableSettings, setColumnFilters } = useTable({
    ...
    initialSettings: {
      ...
      tableSettings: {},
      columnVisibility: { tenantUID: false }
    }
  });
