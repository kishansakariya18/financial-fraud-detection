...
export function RowActions({ row, table }) {
  ...
  const confirmMessages = {
    pending: {
      ...
      description: t('tenant_status_desc'),
      actionText: t('submit')
    },
    success: {
      title: t('tenant') + ' ' + t('status') + ' ' + t('changed'),
      description: t('tenant_status_suceess')
    }
  };
  ...
  const handleChangeStatus = useCallback(async () => {
    ...
    const result = await TenantService.changeTenantStatus(row.original?.tenantUID);
        ...
        <Menu as="div" className="relative inline-block text-left">
          ...
          <Transition
            ...
            <MenuItems
              ...
              {hasPermission(PERMISSIONS.TENANT.CHANGE_STATUS) && (
                <MenuItem>
                  {({ focus }) => (
                    <button
                      onClick={openModal}
