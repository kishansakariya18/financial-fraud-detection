...
import PayemntProviderService from '../../../../services/payment-provider-config.services';
import { getQueryParams, isEmptyObject } from 'utils/custom.utilities';
import { useTranslation } from 'react-i18next';
import useTable from 'components/ui/useTable';
import { DEFAULT_PAGE_INDEX, DEFAULT_PER_PAGE_RECORD } from 'constants/app.constant';
import { paymentProviderResponseMapper, statusToAPI } from '../helper';

export default function PaymentProviderConfig() {
  const { t } = useTranslation();
  const [searchParams, setSearchParams] = useSearchParams();
  const pageTitle = t('paymentProvider');
  const queryParams = useMemo(() => getQueryParams(searchParams), [searchParams]);

  const fetchPaymentProviders = async () => {
    const pageIndex = isNaN(queryParams.pageIndex) ? DEFAULT_PAGE_INDEX : +queryParams.pageIndex;
    const pageSize = isNaN(queryParams.pageSize) ? DEFAULT_PER_PAGE_RECORD : +queryParams.pageSize;
    const result = await PayemntProviderService.paymentProviderList({
    ...
    if (result.status === 200) {
      return {
        status: 200,
        data: paymentProviderResponseMapper(result.response.data),
