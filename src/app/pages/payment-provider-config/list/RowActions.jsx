...
export function RowActions({ row, table }) {
  ...
  const handleChangeStatus = useCallback(async () => {
    ...
    const result = await paymentProviderService.changeProviderStatus(row.original.uid);
  ...
  const handleDelete = useCallback(async () => {
    setConfirmLoading(true);
    const result = await paymentProviderService.deleteProvider(row.original.uid);
    if (result.status === 200) {
  ...
  const handleEdit = useCallback(() => {
    navigate(`/site-configuration/payment-provider-config/edit/${row.original.uid}`);
  }, [navigate, row]);
