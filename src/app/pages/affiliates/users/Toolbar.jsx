...
export function Toolbar({
  ...
  const handleExport = async () => {
    ...
    try {
      setIsExporting(true);
      const res = await AffiliatesService.exportReferredUsersReport({
        affiliateId,
        // userID is optional here; if needed can be threaded from caller
        filters: {
      ...
      if (res.status === 200) {
        ...
        const ts = new Date()
          ...
          .replace('T', '_')
          .replace('Z', '');
        a.download = `referred-users-${affiliateId}-${ts}.csv`;
  ...
  return (
    <div className="table-toolbar">
      <div
        ...
        {hasPermission(PERMISSIONS.AFFILIATES.REPORT?.REFFERED_USER_EXPORT) && (
          <Button
            variant="outlined"
  ...
  return (
    <>
      <Input
        ...
        placeholder={
          t('search') + ' ' + t('campaign') + ' ' + t('id') + ', ' + t('username') + '...'
        }
      />
      <Button
        onClick={() =>
