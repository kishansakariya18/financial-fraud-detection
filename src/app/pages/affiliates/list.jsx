...
import { affiliatesListResponseMapper } from './helper';

export default function AffiliatesList() {
  const { t } = useTranslation();
  const [searchParams, setSearchParams] = useSearchParams();
  const queryParams = useMemo(() => getQueryParams(searchParams), [searchParams]);
  const pageTitle = `${t('affiliates')} ${t('list')}`;

  const fetchData = async () => {
    const pageIndex = isNaN(queryParams.pageIndex) ? DEFAULT_PAGE_INDEX : +queryParams.pageIndex;
    const pageSize = isNaN(queryParams.pageSize) ? DEFAULT_PER_PAGE_RECORD : +queryParams.pageSize;

    const result = await AffiliatesService.getList({
      pagination: { pageIndex, pageSize },
      filters: queryParams
    });

    const apiData = affiliatesListResponseMapper(result.response.data);
  ...
  useEffect(() => {
    ...
    if (queryParams.status) filtersFromQuery.push({ id: 'status', value: queryParams.status });
  ...
  const applyFilterHandler = () => {
    ...
    for (let data of table.getState().columnFilters) {
      ...
      if (data.id === 'status') filterItems.status = data.value;
