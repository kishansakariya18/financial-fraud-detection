      ...
      <div
        ...
        {hasPermission(PERMISSIONS.USER_CLASS_LIMIT.CREATE) && (
          <Button
            ...
            color="primary"
            onClick={() => navigate(`/user-class/${userClassUID}/limits/create`)}>
...
function Filters({ table, onApplyFilters = () => {}, onClearFilters = () => {} }) {
  ...
  return (
    <>
      {table.getColumn('Limit Type') && (
        <FacedtedFilter
          options={limitTypeOptions}
          column={table.getColumn('Limit Type')}
          title={t('limit_type')}
          isMultiple={false}
          showCheckbox={false}
        />
      )}
      {table.getColumn('Limit Period') && (
        <FacedtedFilter
          options={limitPeriodOptions}
          column={table.getColumn('Limit Period')}
