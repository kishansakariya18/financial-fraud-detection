...
import { countryMapper } from '../helper';
...
export default function RestrictedCountry() {
  ...
  const pageTitle = t('country') + ' ' + t('list');
  ...
  const fetchCountry = async () => {
    ...
    const result = await ProviderService.countryList(providerId, {
    ...
    if (result.status === 200) {
      return {
        status: 200,
        data: countryMapper(result.response.data),
  ...
  const onSubmit = async () => {
    ...
    const result = await ProviderService.addRestrictedCountry(providerId, checked);
