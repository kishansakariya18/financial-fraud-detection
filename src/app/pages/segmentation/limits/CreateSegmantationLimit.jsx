...
const CreateSegmentationLimit = () => {
  ...
  const createSegmentationLimit = async (data) => {
    ...
    try {
      ...
      const result = await SegmentationService.addSegmentationLimit(requestData);
  ...
  return (
    <Page title={t('create') + ' ' + t('`segmentation`') + ' ' + t('limit')}>
      <div className="transition-content grid w-full grid-rows-[auto_1fr] px-[--margin-x] pb-8">
        <div className="flex items-center space-x-4 py-5 lg:py-6 rtl:space-x-reverse">
          <h2 className="text-xl font-medium tracking-wide text-gray-800 dark:text-dark-50 lg:text-2xl">
            {t('create') + ' ' + t('segmentation') + ' ' + t('limit')}
          </h2>
...
export default CreateSegmentationLimit;
