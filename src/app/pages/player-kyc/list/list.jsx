...
export default function UserKYC() {
  ...
  const fetchPlayers = async () => {
    ...
    const result = await UserKycServices.getUserKycList({
      pagination: { pageIndex, pageSize },
      filters: queryParams
    });

    const apiData = userKYCResponseMapper(result.response.data.list);

    if (result.status === 200) {
      return {
        status: 200,
        data: apiData,
        totalRecords: parseInt(result?.response?.data?.total_record) || DEFAULT_PER_PAGE_RECORD
      };
    }
