  ...
  return (
    <>
      {(hasPermission(PERMISSIONS.BANNER.LIST) ||
        ...
        hasPermission(PERMISSIONS.BANNER.CHANGE_STATUS) ||
        hasPermission(PERMISSIONS.BANNER.DELETE)) && (
                ...
                {hasPermission(PERMISSIONS.BANNER.EDIT) && (
                  <MenuItem>
                    {({ focus }) => (
                      <button
                        onClick={() =>
                          navigate(`/content-management/banner/${row.original.id}/edit`)
                        }
                        className={clsx(
                          'flex h-9 w-full items-center space-x-3 px-3 tracking-wide text-this outline-none transition-colors dark:text-this-light rtl:space-x-reverse',
                          focus && 'bg-this/10 dark:bg-this-light/10'
                  ...
                  <MenuItem>
                    {({ focus }) => (
                      <button
                        ...
                        <span>{`${t('delete_text')}  ${t('banner')}`}</span>
                      </button>
                    )}
                  </MenuItem>
                )}
