...
export function RowActions({ row, table }) {
  ...
  const confirmMessages = {
    pending: {
      description: t('event_template_desc'),
      actionText: t('submit')
    },
    success: {
      title: t('success'),
      description: t('event_template_status_success')
    }
  };
  ...
  const handleChangeStatus = useCallback(async () => {
    ...
    const result = await EventTemplateService.eventTemplateStatus(row.original.id);
                  ...
                  <MenuItem>
                    {({ focus }) => (
                      <button
                        ...
                        onClick={() => navigate(`/event-template/${row.original.id}/edit`)}>
