...
export function RowActions({ row, table }) {
  ...
  const confirmMessages = {
    pending: {
      description: t('calling_agent_status_desc'),
      actionText: t('submit')
    },
    success: {
      title: t('calling_agent') + ' ' + t('status') + ' ' + t('changed'),
      description: t('calling_agent_status_success')
    }
  };
  ...
  const handleClickView = () => {
    navigate(`/calling-agents/list/${row.original.adminUID}/tab/details`);
  };
  ...
  const handleDeleteRows = useCallback(async () => {
    ...
    const result = await AgentService.changeAgentStatus(row.original.adminUID);
        ...
        <Menu as="div" className="relative inline-block text-left">
          ...
          <Transition
            ...
            <MenuItems
              ...
              {hasPermission(PERMISSIONS.CALLING_AGENT.LIST) && (
                <MenuItem>
                  {({ focus }) => (
                    <button
                      className={clsx(
              ...
              {/* <MenuItem>
                {({ focus }) => (
                  <button
                    ...
                    onClick={() => navigate(`/calling-agents/list/${row.original.adminUID}/edit`)}>
                    <PencilIcon className="size-4.5 stroke-1" />
                    <span>{t('edit')}</span>
                  </button>
                )}
              </MenuItem> */}
              {hasPermission(PERMISSIONS.AGENTS.CHANGE_STATUS) && (
                <MenuItem>
                  {({ focus }) => (
                    <button
                      onClick={openModal}
