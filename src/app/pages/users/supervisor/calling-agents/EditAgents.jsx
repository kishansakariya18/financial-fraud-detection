...
const EditAgents = () => {
  ...
  const fetchSupervisorDetails = async () => {
    const result = await AgentService.getAgentDetail(supervisorUID);
  ...
  const editAgentApi = async (requestObject) => {
    setLoading(true);
    setError(null);
    const result = await AgentService.editAgent(requestObject);
    if (result) {
      if (result.status === 200 || result.status === 201) {
  ...
  const onSubmit = async (data) => {
    await editAgentApi({ ...data, adminUID: supervisorUID });
  };
  return (
