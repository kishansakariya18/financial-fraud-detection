...
import { createSupervisorSchema } from './schema';
...
const CreateAdmin = () => {
  ...
  const {
    ...
    resolver: yupResolver(createSupervisorSchema)
  });
  ...
  const createSupervisorAPI = async (requestObject) => {
    setLoading(true);
    setError(null);
    const result = await SupervisorService.createSupervisor(requestObject);
    if (result) {
      if (result.status === 200 || result.status === 201) {
  ...
  const onSubmit = async (data) => {
    await createSupervisorAPI(data);
  };
  return (
    <Page title={t('create') + ' ' + t('supervisor')}>
      <div className="transition-content grid w-full grid-rows-[auto_1fr] px-[--margin-x] pb-8">
        <div className="flex items-center space-x-4 py-5 lg:py-6 rtl:space-x-reverse">
          <h2 className="text-xl font-medium tracking-wide text-gray-800 dark:text-dark-50 lg:text-2xl">
            {t('create') + ' ' + t('supervisor') + ' ' + t('form')}
          </h2>
