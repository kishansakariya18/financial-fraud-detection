...
const CommissionSettings = () => {
  ...
  const fetchCommissionSettings = async () => {
    setError('');
    const res = await GlobalCommissionSettingService.getCommissionSettings({ affiliateId });
    if (res) {
  ...
  const updateCommissionSettings = async (requestObject) => {
    setLoading(true);
    setError('');
    const res = await GlobalCommissionSettingService.updateCommissionSettings(requestObject);
    if (res) {
      if (res.status === 200 || res.status === 201) {
