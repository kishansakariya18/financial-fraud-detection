...
export function RowActions({ row, table }) {
  ...
  const handleChangeStatus = useCallback(async () => {
    ...
    const result = await smsProviderService.changeProviderStatus(row.original.uid);
  ...
  const handleDelete = useCallback(async () => {
    setConfirmLoading(true);
    const result = await smsProviderService.deleteProvider(row.original.uid);
    if (result.status === 200) {
  ...
  const handleEdit = useCallback(() => {
    navigate(`/site-configuration/sms-provider/edit/${row.original.uid}`);
  }, [navigate, row]);
