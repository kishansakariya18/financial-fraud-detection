...
function SearchInput({ table, onApplyFilters }) {
  return (
    <Input
      value={table?.getColumn('gatewayName')?.getFilterValue() || ''}
      onChange={(e) => table.getColumn('gatewayName').setFilterValue(e.target.value)}
      onKeyDown={(e) => {
        if (e.key === 'Enter') {
          onApplyFilters();
        }
      ...
      placeholder={t('payment_provider_search_desc')}
    />
  );
}
