  ...
  const handleBlockUnblock = useCallback(
    async (reason) => {
      ...
      try {
        if (isBlocked) {
          // Unblock
          result = await CountryService.blockModule(row.original.moduleUID, 0, countryId, reason);
        } else {
          // Block
          result = await CountryService.blockModule(row.original.moduleUID, 1, countryId, reason);
        }
              ...
              <MenuItem>
                {({ focus }) => (
                  <button
                    ...
                    <span>
                      {isBlocked ? t('unblock') : t('block')} {t('module')}
                    </span>
                  </button>
                )}
              </MenuItem>
