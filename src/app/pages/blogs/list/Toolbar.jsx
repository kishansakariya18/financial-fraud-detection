...
export function Toolbar({
  ...
  useEffect(() => {
    ...
    const fetchCategories = async () => {
      try {
        const { response } = await BlogCategoryService.getBlogCategoryList({
        ...
        const options = Array.isArray(response?.data)
          ? response.data.map((category) => ({
              value: Number(category.BlogCategoryID),
              label: category.Name
            }))
          : [];
...
function Filters({ table, onApplyFilters = () => {}, onClearFilters = () => {}, categoryOptions }) {
