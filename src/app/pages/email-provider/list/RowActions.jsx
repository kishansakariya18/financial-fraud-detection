...
export function RowActions({ row, table }) {
  ...
  const handleChangeStatus = useCallback(async () => {
    ...
    const result = await EmailProviderService.changeProviderStatus(row.original.uid);
  ...
  const handleDelete = useCallback(async () => {
    setConfirmLoading(true);
    const result = await EmailProviderService.deleteProvider(row.original.uid);
    if (result.status === 200) {
  ...
  const handleEdit = useCallback(() => {
    navigate(`/site-configuration/email-provider/edit/${row.original.uid}`);
  }, [navigate, row]);
